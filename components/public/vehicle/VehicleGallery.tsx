...
export default function VehicleGallery({ images, title }: VehicleGalleryProps) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="aspect-[4/3] sm:aspect-[16/9] bg-gray-100 rounded-xl flex items-center justify-center">
  ...
  return (
    <>
          ...
          {sorted.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/40 hover:bg-black/60 rounded-full flex items-center justify-center text-white"
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/40 hover:bg-black/60 rounded-full flex items-center justify-center text-white"
      ...
      {/* ── DESKTOP: grid layout with side images ── */}
      <div className="hidden md:block space-y-2">
        <div className="grid grid-cols-3 gap-2 rounded-xl overflow-hidden h-[380px] lg:h-[420px]">
      ...
      {lightboxOpen && (
        ...
        >
          ...
          {sorted.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white"
              >
                <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6" />
              </button>
              <button
                onClick={e => { e.stopPropagation(); lightboxNext() }}
                className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white"
              >
          ...
          <div
            className="relative w-full h-full max-w-5xl max-h-[80vh] mx-12 sm:mx-16"
          ...
          {sorted.length > 1 && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 overflow-x-auto max-w-xs sm:max-w-lg px-4">
              {sorted.map((img, idx) => (
                <button
                  key={idx}
                  onClick={e => { e.stopPropagation(); setLightboxIndex(idx) }}
                  className={`relative w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
